// check-render — the framework's own test gate (decision #9). Renders every shipped
// skill across an adapter matrix and asserts the invariants that keep the convergence
// model honest. Run before committing any framework change (`pnpm gate`);
// improve-framework runs this before opening a PR.
...
const adaptersByAxis: Record<string, string[]> = {}
...
const matrix: FrameworkConfig[] = [
  { harnesses: ['claude-code'] },
  { harnesses: ['claude-code'], orm: 'drizzle', ui: 'shadcn', stack: 'generic-node' },
  { harnesses: ['claude-code'], orm: 'mikro-orm', ui: 'open-mercato-ui', stack: 'generic-node' },
]
for (const axis of Object.keys(adaptersByAxis))
  for (const a of adaptersByAxis[axis] ?? []) matrix.push({ harnesses: ['claude-code'], [axis]: a })
...
const declaredSlots: Record<string, string[]> = {}
for (const s of skills) {
  const body = readFileSync(join(ROOT, 'core/ai/skills', s, 'SKILL.md'), 'utf8')
  declaredSlots[s] = body
    .split('\n')
    .map((l) => l.match(SLOT_OPEN))
    .filter((m): m is RegExpMatchArray => Boolean(m))
    .map((m) => m[1] as string)
}

// Every declared slot must be fillable by at least one adapter on its axis (typo guard).
const adapterSlotNames = (axis: string): Set<string> => {
  const names = new Set<string>()
  for (const a of adaptersByAxis[axis] ?? []) {
    const ad = JSON.parse(readFileSync(join(ROOT, 'adapters', axis, a, 'adapter.json'), 'utf8')) as {
...
for (const s of skills) {
  for (const slot of declaredSlots[s] ?? []) {
    const axis = slot.split('.')[0] as string
    checks++
    if (!adapterSlotNames(axis).has(slot))
      fail(`${s}: slot '${slot}' is filled by no ${axis} adapter (typo or missing adapter content)`)
  }
}
...
for (const s of skills) {
  for (const sel of matrix) {
    ...
    if (a.digest !== b.digest)
      fail(`${s} @ ${JSON.stringify(sel)}: non-deterministic (${a.digest} != ${b.digest})`)
    if (a.rendered.split('\n').some((l) => SLOT_ANY.test(l)))
      fail(`${s} @ ${JSON.stringify(sel)}: leftover SLOT marker (neither filled nor pruned)`)
  }
}

...
3. Writes a flat, concrete skill carrying a `<!-- generated by aef init; edits tracked -->`
...
- `aef init` — interactive or `--harness=claude-code,codex,…`, `--orm=`, `--ui=`, `--stack=`.
  ...
  files + skills dir.
- `aef sync` — recomposes skills/templates from the installed framework version and runs a
...
- `aef add <adapter>` / `aef remove <adapter>` — re-render the affected skills with the
...
5. Runs `aef sync` in the consumer so the fix flows back down via the normal 3-way merge —
                                   ...
                                   [SCHEDULED] improve-framework → draft PR ──▶ framework repo
                                                                    ...
                                                                    [SCHEDULED] triage-feedback (dedup)
                                                                            ...
                                                                            framework source + lessons.framework.md
                                                                                     │
   all consumers ◀── human merge ◀── [SCHEDULED] aef sync opens a local PR ◀─────┘
```

---

## 4c. Developing the framework itself (decision #9)

The framework repo is the one place **source and usage coincide** — the same situation
open-mercato had as a monorepo. It therefore can't be `init`'d like a normal consumer (that would
...
**The "meta" install.** `aef dev` symlinks the framework's own harness dirs
(`.claude/skills`, `.codex/skills`, `.cursor/skills`) to the **dev skills' source**. No rendering,
no adapter selection (a maintainer works across all adapters), no committed render artifacts. The
harness dirs are **gitignored** (anchored to root so `examples/` stays tracked) and regenerated by
`aef dev` — answering "do we install meta?": yes, run `aef dev` after cloning, the way
...
- **Phase 5 — Sync + provenance:** build `aef sync` (+`add`/`remove`) with git-native 3-way
  merge; ensure the renderer is deterministic and emits the provenance manifest. _(PoC landed:
  `aef sync` via `git merge-file` against a `.ai/.base/` snapshot; `add`/`remove` still ahead.)_
...
- **The framework repo develops in "dev mode":** it's a special consumer (source == usage), so
  `aef dev` installs the _dev_ toolchain skills (not the shipped product), and changes are

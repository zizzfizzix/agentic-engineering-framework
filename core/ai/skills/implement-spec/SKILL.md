...
1. **Identify the spec**: Locate the target spec file(s) under the specs root (`aef.config.json` → `paths.specsRoot`; include any `enterprise/` subfolder if the project uses one).
...
- **App-level module**: Place code under the modules root (`aef.config.json` → `paths.modulesRoot`, e.g. `<modulesRoot>/<module>/`), or the user's app repo.
...
- Verify tests pass using the test command listed in `aef.config.json` → `validation` (scoped to the affected module where the runner supports it)
...
- Run and verify with the integration test runner configured under the tests root (`aef.config.json` → `paths.testsRoot`), e.g. running the project's test/e2e command against the new test path with retries disabled
...
- If new convention files: run the project's code-generation/prepare command (see `aef.config.json` → `validation` and project AGENTS.md)
...
After all targeted phases are complete, run the verification gate — the commands listed in `aef.config.json` → `validation` (typically a build, lint, typecheck, and test pass) — and:

1. **Verification gate**: run every command in `aef.config.json` → `validation` — all must pass
...
- MUST run the verification gate (`aef.config.json` → `validation`) after the final phase to verify no build/lint/type/test breaks

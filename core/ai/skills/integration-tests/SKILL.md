...
| Test files location                                                              | `<module>/__integration__/TC-XXX.spec.ts` (under `aef.config.json` → `paths.modulesRoot`)       |
...
- Keep global settings in the Playwright config under the tests root (`aef.config.json` → `paths.testsRoot`, e.g. `<testsRoot>/playwright.config.ts`):
  - `timeout: 10_000`
...
```bash
# <qa> is the scenarios folder alongside the tests root; <modulesRoot> is aef.config.json -> paths.modulesRoot
...
Create the test in the module where the behavior lives, under the modules root (`aef.config.json` → `paths.modulesRoot`). The locations below are examples from one platform's monorepo layout:
...
Run the new test to confirm it passes (using the Playwright config under the tests root, `aef.config.json` → `paths.testsRoot`):
...
- MUST place executable tests in module-local `__integration__` directories (under `aef.config.json` → `paths.modulesRoot`); never add `.spec.ts` files under the tests root itself
...
Given a spec like SPEC-017 (Version History Panel), the skill would produce (paths under `aef.config.json` → `paths.modulesRoot`; the `<modulesRoot>/admin/...` layout below is illustrative):
...
The `yarn test:integration*` commands below are one platform's `package.json` scripts; use the integration/e2e commands from `aef.config.json` → `validation` and the Playwright config under the tests root (`aef.config.json` → `paths.testsRoot`).

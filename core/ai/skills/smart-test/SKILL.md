...
The path patterns below are examples from one platform's monorepo layout (`packages/...`, `apps/mercato/...`); substitute your project's structure (`framework.config.json` → `paths.modulesRoot` for modules) and shared/foundational packages.
...
root. For example, an app module and a scaffolding-template module that share the same name
(e.g. `apps/mercato/src/modules/example` and a `create-app/template/src/modules/example` — one
platform's example) are separate `example` modules, so an app-specific page change does not
trigger template integration specs.
...
Totals come from `references/test-architecture.md` (the approximate suite counts recorded there for this project). Round to one decimal place.

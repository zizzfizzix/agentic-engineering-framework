...
default branch (`aef.config.json` → `git.defaultBranch`) can pull in unrelated shared-package
...
If the diff against the configured default branch (`aef.config.json` → `git.defaultBranch`,
...
The path patterns below are examples from a typical monorepo layout (`packages/...`, `apps/<app>/...`); substitute your project's structure (`aef.config.json` → `paths.modulesRoot` for modules) and shared/foundational packages.
...
**Wide scope fallback**: when `CHANGED` includes shared/events/queue/cache files, run the full unit suite instead (the test command from `aef.config.json` → `validation`, e.g. `yarn test`).
...
**If the server is NOT running**: build the project and start the production server (the `yarn build` / `yarn start` commands below are examples — use the project's build command from `aef.config.json` → `validation` and its start command):
...
The script lives in this skill's `scripts/` directory; the integration commands below are examples — use the integration test command from `aef.config.json` → `validation` and the Playwright config under the tests root (`aef.config.json` → `paths.testsRoot`).
